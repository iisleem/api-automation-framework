...
def _should_open_via_http_server(report_path: Path) -> bool:
    return _is_official_allure_report(report_path) or _is_environment_matrix_dashboard(report_path)
...
def _is_environment_matrix_dashboard(report_path: Path) -> bool:
    return (
        report_path.name == "index.html"
        and report_path.parent.name == "environment-matrix"
