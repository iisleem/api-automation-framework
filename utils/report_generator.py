...
def generate_environment_matrix_dashboard(
    environment_runs: list[dict[str, Any]],
    output_dir: Path,
) -> Path:
    output_dir.mkdir(parents=True, exist_ok=True)
    report_path = output_dir / "index.html"
    report_path.write_text(_render_environment_matrix_html(environment_runs), encoding="utf-8")
    return report_path
...
def _render_environment_matrix_html(environment_runs: list[dict[str, Any]]) -> str:
    totals = _matrix_totals(environment_runs)
    cards = "\n".join(_render_environment_card(run) for run in environment_runs)
    rows = "\n".join(_render_environment_row(run) for run in environment_runs)
    failures = "\n".join(_render_failure_item(run) for run in environment_runs for _ in [0])
    if not failures:
        failures = "<li>No environment-level failures detected.</li>"

    return f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>API Environment Matrix Dashboard</title>
  <style>
...
<body>
  <header>
    <h1>API Environment Matrix Dashboard</h1>
    <p>One dashboard for the full multi-environment API automation run, with drill-down reports per environment.</p>
  </header>
  <main>
    <section class="summary">
      <div class="metric"><strong>{totals["environments"]}</strong>Environments</div>
...
def _matrix_totals(environment_runs: list[dict[str, Any]]) -> dict[str, Any]:
    totals = {
        "environments": len(environment_runs),
        "total": 0,
        "passed": 0,
        "failed": 0,
        "broken": 0,
...
def _render_environment_card(run: dict[str, Any]) -> str:
    summary = run["summary"]
    env = html.escape(run["env"])
  ...
  <a href="{report_href}">Open {env} report</a>
  <br>
  <a href="{log_href}">Open execution log</a>
</article>"""


def _render_environment_row(run: dict[str, Any]) -> str:
    summary = run["summary"]
    env = html.escape(run["env"])
    report_href = html.escape(run["report_href"])
    log_href = html.escape(run.get("log_href", "#"))
...
def _render_failure_item(run: dict[str, Any]) -> str:
    ...
    return f'<li>{env}: {count} failed/broken tests. <a href="{report_href}">Open report</a></li>'
