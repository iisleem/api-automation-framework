...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="framework.py",
        description="Unified CLI for the API automation framework.",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    run_parser = subparsers.add_parser("run", help="Run API tests locally or as an environment matrix.")
    run_parser.add_argument("--env", default=None, help="Environment from config/environments.yaml.")
    run_parser.add_argument("--envs", nargs="+", help="Run an API environment matrix.")
    run_parser.add_argument("--env-workers", type=int, help="Parallel environment suites for matrix execution.")
    run_parser.add_argument("--base-url", help="Override the environment REST API base URL.")
    run_parser.add_argument("--graphql-url", help="Override the environment GraphQL URL.")
    ...
    run_parser.add_argument("--contract", action="store_true", help="Run API contract tests.")
    run_parser.add_argument("--rest", action="store_true", help="Run REST API tests.")
    run_parser.add_argument("--graphql", action="store_true", help="Run GraphQL API tests.")
    run_parser.add_argument("--auth", action="store_true", help="Run authentication and authorization tests.")
...
def _run_tests(args: argparse.Namespace, extra_pytest_args: list[str]) -> int:
    ...
    environments = _resolve_run_environments(args)
    use_matrix = args.matrix or bool(args.envs) or len(environments) > 1

    if use_matrix:
        command = [
            sys.executable,
            str(PROJECT_ROOT / "scripts" / "run_environment_matrix.py"),
            "--envs",
            *environments,
        ]
    ...
    command = [sys.executable, "-m", "pytest", "--env", environments[0]]
    if args.base_url:
        command.extend(["--base-url", args.base_url])
    if args.graphql_url:
        command.extend(["--graphql-url", args.graphql_url])
...
def _resolve_run_environments(args: argparse.Namespace) -> list[str]:
    if args.env and args.envs:
        raise SystemExit("Use either --env for a normal run or --envs for a matrix run, not both.")
    if args.envs:
        return args.envs
    if args.matrix:
        settings = ConfigReader(PROJECT_ROOT).read_settings()
        return settings.get("execution", {}).get("environments", ["mock"])
    return [args.env or "mock"]
...
def _open_helpers_catalog(args: argparse.Namespace) -> int:
    ...
    if not catalog_path.exists():
        ...
        return 1
    return 0 if open_report(catalog_path, LOGGER) else 1
...
def _check_python(doctor: Doctor) -> None:
    ...
    if version >= (3, 11):
        ...
        doctor.fail(f"Python 3.11+ is required. Current: {version.major}.{version.minor}.{version.micro}")
...
def _check_project_files(doctor: Doctor) -> None:
    required_files = [
        ...
        "data/users.json",
        "data/payloads/create_booking.json",
        "conftest.py",
        "pytest.ini",
        "requirements.txt",
        "scripts/run_environment_matrix.py",
        "docs/helpers_catalog.html",
        "docs/FRAMEWORK_HELPERS.md",
        "docs/FEATURE_PARITY.md",
        "schemas/rest/health.schema.json",
...
def _check_yaml_config(doctor: Doctor) -> None:
    ...
    try:
        ...
        configured_envs = settings.get("execution", {}).get("environments", [])
        missing_envs = [env for env in configured_envs if env not in environments]
        if missing_envs:
            doctor.fail(f"Invalid configured environment(s): {', '.join(missing_envs)}")
        else:
...
def _check_api_env(doctor: Doctor) -> None:
    if not os.getenv("API_BEARER_TOKEN") and not os.getenv("API_KEY"):
        doctor.warn("API_BEARER_TOKEN and API_KEY are not set. Required only for authenticated live API scenarios.")
    else:
        doctor.pass_("API authentication environment variables are available")
